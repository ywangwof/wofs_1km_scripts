...
source /scratch/wofs_1km/wofs_1km_scripts/WOFenv_d02
...
while (1 == 1)
  ...
  if ( ${startdate} == ${datea} ) then
    ...
    ${COPY} ${TEMPLATE_DIR}/obs_locinfo_multi_1km ${ENKFDIR}/obs_locinfo
  ...
  if( ${PRIOR_INF} == 1) then
   # check whether the prior_inf.nc and prior_in_sd.nc exist, if one of them does not exist, create new copies for both
    if ((! -e ${ENKFDIR}/prior_inf_d${DOMAIN}.1) || (! -e ${ENKFDIR}/prior_inf_sd_d${DOMAIN}.1)) then # one background
     ...
     echo '#SBATCH' "-J runinitinf2"                                              >> ${ENKFDIR}/runinitinf.job
     ...
     echo '#SBATCH' "--ntasks-per-node=20"                                       >> ${ENKFDIR}/runinitinf.job
...
#    ${REMOVE} runinitinf.err
   ...
   echo "running prior inflation"
     ...
     echo '#SBATCH' "-J runpriorinf2"                                             >> ${ENKFDIR}/runpriorinf.job
...
######################### RUN GSI.exe ###########################
   ...
   if ( -e ${SAT_DIR}/RADIANCE/3KM/${startyear}/${thisstart}_GOES${GOESV}_TB_ALLSKY.nc ) then
      /bin/cp -pv ${SAT_DIR}/RADIANCE/3KM/${startyear}/${thisstart}_GOES${GOESV}_TB_ALLSKY.nc ${ENKFDIR}/goes.nc
      set goesyes = 1
   endif
...
########################### PERFORM ENKF DA ##############################
  ...
  echo "running EnKF"
     ...
     echo '#SBATCH' "-J runenkf2"                                                 >> ${ENKFDIR}/runenkf.job
     ...
     echo '#SBATCH' "--ntasks-per-node=20"                                       >> ${ENKFDIR}/runenkf.job
     echo '#SBATCH' "-n 600"                                            >> ${ENKFDIR}/runenkf.job
...
EOF
  ...
  ${MOVE} ${ENKFDIR}/wrfinput_d02.? ${RUNDIR}/${thisstart}
  ${MOVE} ${ENKFDIR}/wrfinput_d02.?? ${RUNDIR}/${thisstart}
...
#    ${MOVE} ${RUNDIR}/${datea}/*err
   ...
   cd ${ENKFDIR}
   ${REMOVE} ${ENKFDIR}/wrfinput_d02*ensmean
   ${COPY} ${SCRIPTDIR}/run_nceamean_conv_d02.job .
   sbatch run_nceamean_conv_d02.job
