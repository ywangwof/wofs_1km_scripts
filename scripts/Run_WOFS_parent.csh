  ...
  if( ${PRIOR_INF} == 1) then
   # check whether the prior_inf.nc and prior_in_sd.nc exist, if one of them does not exist, create new copies for both
    if ((! -e ${ENKFDIR}/prior_inf_d${DOMAIN}.1) || (! -e ${ENKFDIR}/prior_inf_sd_d${DOMAIN}.1)) then # one background
     ...
     echo '#SBATCH' "-A largequeue"                                              >> ${ENKFDIR}/runinitinf.job
     echo '#SBATCH' "-p workq"                                                   >> ${ENKFDIR}/runinitinf.job
     echo '#SBATCH' "--ntasks-per-node=24"                                       >> ${ENKFDIR}/runinitinf.job
...
EOF
   ...
   echo "running prior inflation"
     ...
     echo '#SBATCH' "-A largequeue"                                              >> ${ENKFDIR}/runpriorinf.job
     echo '#SBATCH' "-p workq"                                                   >> ${ENKFDIR}/runpriorinf.job
     echo '#SBATCH' "--ntasks-per-node=16"                                       >> ${ENKFDIR}/runpriorinf.job
     echo '#SBATCH' "-n 80"                                            >> ${ENKFDIR}/runpriorinf.job
     echo '#SBATCH' "-t 00:10:00"                                                >> ${ENKFDIR}/runpriorinf.job
     echo "#=================================================================="  >> ${ENKFDIR}/runpriorinf.job
     ...
     setenv MPICH_GNI_RDMA_THRESHOLD 2048
     setenv MPICH_GNI_DYNAMIC_CONN disabled
     #setenv MPICH_MPIIO_HINTS "${RUNDIR}/EXP/enkfrun/wrfout*:striping_factor=4, cb_nodes=4"
     setenv MPICH_CPUMASK_DISPLAY 1
...
#       python ${ENKFDIR}/dbz_boundary.py
       set reflyes = 1 
       ${COPY} ${ADDNOISEDIR}/grid_refl_obs_gsi ./
       ${COPY} ${SCRIPTDIR}/run_grid_refl.job ./ 
       if ( -e ${ENKFDIR}/grid_refl_obs_gsi ) then#
...
${REMOVE} advModel.sed gsiparm.anl.head
   ...
   echo '#SBATCH' "-A largequeue"                                              >> ${ENKFDIR}/run_gsi_mem.job 
   echo '#SBATCH' "-p workq"                                                   >> ${ENKFDIR}/run_gsi_mem.job
   echo '#SBATCH' "--ntasks-per-node=24"                                       >> ${ENKFDIR}/run_gsi_mem.job
   echo '#SBATCH' "-n ${GSI_CORES}"                                            >> ${ENKFDIR}/run_gsi_mem.job
   echo '#SBATCH -t 0:10:00'                                                   >> ${ENKFDIR}/run_gsi_mem.job
   echo "#=================================================================="  >> ${ENKFDIR}/run_gsi_mem.job
   ...
   sleep 2 
   srun ${RUNDIR}/GSI_RUN/gsi.exe > ${ENKFDIR}/run_gsi.mem\${SLURM_ARRAY_TASK_ID}_${datea}.log
   sleep 2
...
#		 exit(0)
   ...
   cd ${ENKFDIR}
   ${COPY} ${GSIDIR}/util/Analysis_Utilities/read_diag/innov_mean_conv.exe ${ENKFDIR}/innov_mean_conv.exe
   ${COPY} ${GSIDIR}/util/Analysis_Utilities/read_diag/innov_mean_radiance.exe ${ENKFDIR}/innov_mean_radiance.exe
...
########################### PERFORM ENKF DA ##############################
  ...
  echo "running EnKF"
     ...
     echo '#SBATCH' "-A largequeue"                                              >> ${ENKFDIR}/runenkf.job
     echo '#SBATCH' "-p workq"                                                   >> ${ENKFDIR}/runenkf.job
     echo '#SBATCH' "--ntasks-per-node=${EKF_NODES}"                                       >> ${ENKFDIR}/runenkf.job
     echo '#SBATCH' "-n ${EKF_CORES}"                                            >> ${ENKFDIR}/runenkf.job
     echo '#SBATCH' "-t 0:25:00"                                                 >> ${ENKFDIR}/runenkf.job
     echo "#=================================================================="  >> ${ENKFDIR}/runenkf.job
...
#### Wait for nest DA cycle to finish before advancing
     ...
     set imem = 1
     while ( ${imem} <= ${ENS_SIZE} )
        while ( ! -e ${CENTRALDIR}/D02/${event}/${thisstart}/wrfinput_d02.${imem} )
         echo "WAITING FOR NEST DA TO FINISH"
         sleep 10
        end
...
# SET ADDITIVE NOISE FLAG TO FALSE FOR FIRST CYELE
     ...
     echo '#SBATCH' "-A largequeue"                                              >> ${RUNDIR}/adv_wrf_mem.job
     echo '#SBATCH' "-p workq"                                                   >> ${RUNDIR}/adv_wrf_mem.job
     echo '#SBATCH' "--ntasks-per-node=24"                                       >> ${RUNDIR}/adv_wrf_mem.job 
     echo '#SBATCH' "-n ${WRF_CORES}"                                            >> ${RUNDIR}/adv_wrf_mem.job
     echo '#SBATCH' "-t 0:30:00"                                                 >> ${RUNDIR}/adv_wrf_mem.job 
     echo "#=================================================================="  >> ${RUNDIR}/adv_wrf_mem.job
     ...
     setenv MPICH_GNI_RDMA_THRESHOLD 2048
     setenv MPICH_GNI_DYNAMIC_CONN disabled
     setenv MPICH_MPIIO_HINTS "${RUNDIR}/enkfrun\${SLURM_ARRAY_TASK_ID}/wrfout*:striping_factor=4, cb_nodes=4"
     setenv MPICH_CPUMASK_DISPLAY 1
     ...
     if ( ${noise_flag} == 1 ) then 
        ./add_pert_where_high_refl refl_obs.txt wrfinput_d01 $lh $lv $u_sd $v_sd $w_sd $t_sd $td_sd $qv_sd ${iseed} ${iseed2} \${SLURM_ARRAY_TASK_ID}  > ./addnoise.output.\${SLURM_ARRAY_TASK_ID}
     endif
     ...
     if ( -e ${RUNDIR}/enkfrun\${SLURM_ARRAY_TASK_ID}/wrfout_d01_${endyear}-${endmonth}-${endday}_${endhour}:${endmin}:00 ) then
      ...
      ${MOVE} ${RUNDIR}/enkfrun\${SLURM_ARRAY_TASK_ID}/wrfout_d02_${startyear}-${startmonth}-${startday}_${starthour}:${startmin}:00 ${CENTRALDIR}/D02/${event}/${thisstart}/wrfout_d02_${startyear}-${startmonth}-${startday}_${starthour}:${startmin}:00_\${SLURM_ARRAY_TASK_ID}
      ${COPY} ${RUNDIR}/enkfrun\${SLURM_ARRAY_TASK_ID}/wrfout_d02_${endyear}-${endmonth}-${endday}_${endhour}:${endmin}:00 ${CENTRALDIR}/D02/${event}/${thisstart}/wrffcst_d02_${endyear}-${endmonth}-${endday}_${endhour}:${endmin}:00_\${SLURM_ARRAY_TASK_ID}
      ${MOVE} ${RUNDIR}/enkfrun\${SLURM_ARRAY_TASK_ID}/wrfout_d02_${endyear}-${endmonth}-${endday}_${endhour}:${endmin}:00 ${CENTRALDIR}/D02/${event}/enkfdir/wrfinput_d02.\${SLURM_ARRAY_TASK_ID}
